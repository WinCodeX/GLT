class AddPaymentFieldsToPackages < ActiveRecord::Migration[7.0]
  def change
    ...
    add_column :packages, :payment_status, :string, default: 'unpaid', null: false
    ...
    # M-Pesa payment tracking fields (if not already present)
    ...
    add_index :packages, :payment_request_id
    add_index :packages, :collected_by_id
    add_index :packages, [:payment_type, :payment_status]
    
    # Foreign key for collected_by
